...
phy <- subset_samples(inc.physeq, treatment %in% c("Alfalfa") & day %in% c("0", "7", "14", "21", "35", "49", "97")) %>%
  filter_taxa(function(x) sum(x) >= 3, T) 

colnames(sample_data(phy))

rela.phy <- merge_samples(phy, "day") %>%
  transform_sample_counts(function(x) x / sum(x)) 

sample_data(rela.phy)$day <- levels(sample_data(phy)$day)
sample_data(rela.phy)$treatment <- levels(as.factor(sample_data(phy)$treatment))
...
data <- merge.phy %>%
  select(treatment, day, OTU, Abundance, Responder, Genus) %>%
  filter(treatment == "Alfalfa")
...
ppp <- pp + 
  ...
  ggtitle("Alfalfa responders") 

tabletable <- OTUs %>%
  select(OTU, Phylum, Genus)

ppp + annotation_custom(tableGrob(tabletable, rows = NULL),
                        xmin = 14, xmax = 97, ymin = 0.02, ymax = 0.03)

png("Figures/alfalfa_area_responders.png",height=8,width=10,units='in',res=300)
ppp + annotation_custom(tableGrob(tabletable, rows = NULL),
                        xmin = 14, xmax = 97, ymin = 0.02, ymax = 0.03)
dev.off()
```
