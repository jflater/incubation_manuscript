...
d = vegdist(t(data.frame(otu_table(tmp.ps))),
            method = "bray")
...
tiff("Figures/NMDS_incubation_axes12.tif",height=4,width=9,units='in',res=300)
NMDS.treatment + stat_ellipse(geom = "polygon", type = "norm",
                   alpha = 0.2, aes(fill = day)) + 
  ggtitle("Axes 1 and 2, stress = 0.09")
dev.off()

NMDS.treatment.13 <- plot_ordination(tmp.ps, NMDS.phy, type = "samples", color = "day", axes = c(1, 3))

tiff("Figures/NMDS_incubation_axes13.tif",height=4,width=9,units='in',res=300)
NMDS.treatment.13 + stat_ellipse(geom = "polygon", type = "norm",
                   alpha = 0.2, aes(fill = day)) +
  ggtitle("Axes 1 and 3, stress = 0.09")
dev.off()


NMDS.treatment<- plot_ordination(tmp.ps, NMDS.phy, type = "samples", color = "treatment")

tiff("Figures/NMDS_incubation_treatmentaxes12.tif",height=4,width=9,units='in',res=300)
NMDS.treatment + stat_ellipse(geom = "polygon", type = "norm",
...
tiff("Figures/NMDS_incubation_treatmentaxes13.tif",height=4,width=9,units='in',res=300)
NMDS.treatment.13 + stat_ellipse(geom = "polygon", type = "norm",
                                 alpha = 0.2, aes(fill = treatment)) +
  ggtitle("Axes 1 and 3, stress = 0.09")
dev.off()
...
PCoA.day.34 <- PCoA.day.23 + stat_ellipse(geom = "polygon", type = "norm", alpha = 0.3, aes(fill = day))

tiff("Figures/adonis_incubation_TreatmentAndDay.tif",height=2,width=9,units='in',res=1200)
...
tiff("Figures/adonis_incubation_day*trt.tif",height=2,width=9,units='in',res=1200)
