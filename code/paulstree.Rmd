...
# plot(final_resp) + scale_fill_viridis(discrete = F, option = "viridis") + 
...
resp <- rbind(resp_alf[1:3], resp_comp[1:3], resp_mix[1:3])

dist_all_resp <- distinct(resp)
df <- merge(resp_alf, dist_all_resp, all = T) %>%
  merge(resp_comp, all = T) %>%
  merge(resp_mix, all = T)
```
```{r}
...
plot(final_resp)
plot(final_resp) + scale_color_viridis(discrete = T, option = "viridis") + ggplot2::theme_bw()
png("../Figures/resp_tree.png",height=40,width=14,units='in',res=600)
plot(final_resp) + scale_fill_viridis(discrete = F, option = "viridis") + ggplot2::theme_bw()
  guides(fill=guide_legend(title="Log fold change"))
dev.off()
```
