...
inc.model.data <- lme(C_flash~treatment * day, random=~1|replication 
                      , data = data
...
theme_set(theme_bw())
p <- ggplot(data = data, aes(x = day, y = C_flash)) +
  ...
  xlab("Day") +
  ylab("Total Carbon via combustion analysis") +
...
png("Figures/C_flash.png",height=5,width=5,units='in',res=300)
p
dev.off()
p
...
png("Figures/C_flash.png",height=5,width=8,units='in',res=300)
p
dev.off()
p
