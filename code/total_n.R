...
inc.model.data <- lme(N_flash~treatment * day, random=~1|replication 
                      , data = data
...
theme_set(theme_bw())
p <- ggplot(data = data, aes(x = day, y = N_flash)) +
  ...
  xlab("Day") +
  ylab("Total Nitrogen via combustion analysis") +
...
png("Figures/N_flash.png",height=5,width=5,units='in',res=300)
p
dev.off()
p
...
png("Figures/N_flash.png",height=5,width=8,units='in',res=300)
p
dev.off()
p
